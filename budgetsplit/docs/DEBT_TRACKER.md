...
| ~~All 3 shortcuts need their iCloud links~~ | **Closed 2026-08-09.** `npm run build:shortcuts` generates and Apple-signs all three from `VOICE_COMMANDS`; they were imported, re-shared, and their links are in `VOICE_SHORTCUT_URL` / `VOICE_INCOME_URL` / `VOICE_SETTLE_URL`. ⚠️ **Editing any shortcut invalidates its link** — Apple keeps serving the version that was shared, so any change to the generated actions means rebuild → import → re-share → replace the constant. This has now cost two full rounds; check the constant is current before trusting a bug report about setup. |
| **`Save File`'s destination cannot be generated** | It is a security-scoped bookmark to a folder on the device that authored the shortcut, so `Log expense` needs its destination re-picked once after import (`postImportStep` says so, and the build script prints it). The two app-opening commands have no folder and import complete. |
