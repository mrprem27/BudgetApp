...
| **⚠️ All 3 install links are null again** | Voice moved from file capture to **deep links** (`budgetsplit:///add/quick?kind=…&q=…`), so every shortcut's actions changed and every shared link is dead. Rebuild (`npm run build:shortcuts`, 3/3 signs), import, re-share, paste into `VOICE_SHORTCUT_URL` / `VOICE_INCOME_URL` / `VOICE_SETTLE_URL`. **This is the third round of that dance** — see the warning above about editing invalidating links. |
