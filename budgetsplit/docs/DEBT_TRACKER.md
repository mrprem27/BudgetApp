...
| **⚠️ The shared shortcut predates the retry loop** | `VOICE_SHORTCUT_URL` installs a working shortcut, but one built before the ask-again loop existed — so the setup steps and the flow on the Voice screen describe a retry the installed copy does not have. Rebuild, re-import, re-share, replace the constant. Left live rather than nulled because the old one still captures correctly; only the silence handling is missing. |
| **`WFCondition: 100` is unverified** | The retry hinges on an If reading *"has any value"*, and that condition code is not documented by Apple or pinned down by the community references. `shortcuts sign` validates structure, not semantics, so a wrong code signs cleanly and then behaves oddly. Cheap to check: after importing, the Shortcuts app renders the If row in words — read it. |
