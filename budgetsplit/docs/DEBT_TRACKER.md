...
| **⚠️ `VOICE_SHORTCUT_URL` is null** | The command is now **"Please log"** (two words: a lone common noun competes with Siri's own intents, and that failure is silent). The shared copy is still named "budget", so the phrase this app documents reaches nothing — nulled rather than left live, same call as the earlier rename. It also predates the retry loop. Rebuild → import → share → paste. |
