...
| ~~`WFCondition: 100` is unverified~~ | **Closed 2026-08-10.** `100` = *has any value*, existence category, no extra fields — from a condition-code table documented as verified against an Apple-built sample shortcut ([viticci/shortcuts-playground-plugin `CONTROL_FLOW.md`](https://github.com/viticci/shortcuts-playground-plugin)). Independently corroborated: the shared copy was downloaded back from iCloud and `WFCondition => 100` survived the device round trip intact. Every other identifier we emit was checked against the same reference (`WFExitAction`, `WFOpenURLAction`, `WFAskForInputAction`, output name `Provided Input`). |
...
| ~~The live link predates three fixes~~ | **Closed 2026-08-10.** Re-shared from the rebuilt file; `VOICE_SHORTCUT_URL` now points at a record named exactly `Please log` whose 14 actions were diffed against `shortcutActions` parameter for parameter — URL bound to the encode output, `WFCondition: 100`, read-back above the URL action, closing pair outside the loop. Re-verify that way after any future re-share: a URL wired to the un-encoded variable looks identical from the outside. |
| **`Dismiss Siri` is unverified on device** | Success reads the phrase back (`Got it — …`) before opening the app; three silent tries end with *"Nothing logged."* + `is.workflow.actions.dismisssiri`. The read-back exists because a mis-hearing is the one failure no condition in the shortcut can detect — non-empty text looks identical to correct text. `Dismiss Siri` takes no parameters and signs cleanly, but signing checks structure, not that Siri actually closes. Deliberately **not** on the success path: the app coming to the foreground already dismisses Siri, and inserting an action between the hand-off and the launch risks the main path for something already happening. |
