...
| **The live link predates both fixes** | `VOICE_SHORTCUT_URL` was minted before the URL-encode fix *and* before the filename fix, so what installs today is named `please-log` and splices raw dictation. Neither is fatal for ordinary phrases; both are fixed in `build/shortcuts/Please log.shortcut`. **One re-import + re-share closes both** — do them together rather than as two round trips. |
