...
| **⚠️ The install link is null** | Voice is now **one** shortcut (`budget`) using a deep link, down from three. Rebuild (`npm run build:shortcuts`, 1/1 signs), import, share, paste into `VOICE_SHORTCUT_URL`. One link to maintain from here rather than three, which is most of the reason for collapsing them. |
