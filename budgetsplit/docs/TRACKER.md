# TRACKER.md — everything open, in one place

`Last verified: 2026-09-07 · Guarded by: trackerIntegrity.test.ts · countClaims.test.ts · docIdGraph.test.ts`

Every open finding, decision and deferral in this repo. Before this file there were **nine
registers** across five documents, and they had started to disagree with each other. Four sync
failures carried one status in one file and the opposite in another — and **the disagreements pointed
both ways**: three were fixed in code while one document still called them open, and a fourth was
ticked as done while the code showed it was not. Twenty-five decisions existed in two copies. Every
stated count was wrong. Eleven cross-references pointed at table rows that had been deleted.

That is the reason this file exists rather than a tidier version of the old ones: **a register
nobody can trust is worse than no register**, and nine of them disagreeing is worse still. Every
status below was re-derived from the source tree or from the most recent of the conflicting
documents — not copied.

## §0 · How this file works

**Status.** One vocabulary, replacing the ✅ / ⚠️ / ⏳ / strikethrough / "done DATE" / five-verdicts
mix that came before it.

| Status | Means |
|---|---|
| `OPEN` | Needs work. Has a next action. |
| `DECIDE` | Needs an answer from you before any code. Carries **the default if you never decide**. |
| `BLOCKED` | Waiting on something outside this repo — Apple, Google, a bank, a second phone. Names it. |
| `PARKED` | Deliberately not now, with the **trigger** that un-parks it. |
| `DONE` | Closed, with the date. **Kept, not deleted** — see below. |
...
### The three guards — why this file should not go stale

A tracker rots by being edited by hand and believed anyway. Each of these turns one class of rot
into a failing test rather than something a reader has to notice:
...
3. **No id is defined twice.** `trackerIntegrity.test.ts` allows an id exactly one definition. Every
   contradiction listed at the top of this file came from the same id being maintained in two
   places, and drifting.

**What is not here.** `SYSTEM.md` keeps the entity dictionary, the flow catalog and the invariants;
`SCREENS.md` keeps the screen specs; `SYNC-MODEL.md` keeps the plain-language explanation of what
happens when someone else can change your numbers; `RELEASE_CHECKLIST.md` keeps the device pass and
the ship procedure; `AGENTS.md` keeps the standing rules. Those are reference, not backlog. This
file is the backlog.
...
| `B-06` | **Confirm demo/seed data is off** in release builds. `seedDemo.ts` and the CSV export's hardcoded demo-row signatures drift apart by design. | `OPEN` |
...
| `B-08` | **Privacy policy + App Store listing.** Required even for external TestFlight, and newly sharp: a server now holds email addresses. | `OPEN` |
| `B-09` | **India DPDP posture.** The moment one real user signs in, an email address is personal data on a server you operate. Being opt-in does not change this. Also `DQ-05`. | `DECIDE` |
...
| `B-13` | **Paste the store copy into App Store Connect and confirm the privacy answers.** The draft is rewritten and correct — it distinguishes personal data (which genuinely never leaves the device) from shared groups (which do, sealed, no key on the server). Receipt photos count as collected because they leave the device, and they are ON by default; an undeclared data type is a rejection. **Yours to do.** | `OPEN` |
...
**40 entries: 4 answered and kept, 29 still open, 7 `BLOCKED` outside the codebase** (`DQ-80`–`DQ-86`,
...
| `SYNC-F15` | **Any approved member can push a new version of an entry I authored.** `existing` is looked up by **id alone** — no group check, no authorship check (`peerIngest.ts:184-186`) — so the edit branch rewrites `author_person_id` to them and replaces my payment and share rows. The server authorises *membership*, not authorship. | Bind the lookup to `(id, group_id)` and refuse a version bump on an entry whose author is me. Server-side, authorise the pusher against the stored author. |
| `SYNC-F16` | **Removing a member is local only.** `removeMemberFromGroup` makes no server call: their `sync_member` row stays approved and their key wraps stay, so they keep pulling every future entry. My device refuses their writes; the server was never told. | Removal must call the server, the way leaving already does. Opens with re-keying (`SYNC-F17`). |
| `SYNC-F17` | **The group key is never rotated** — not on leave, not on removal, not on account deletion. Only the leaver's own wraps are dropped. **Anyone who ever held the key holds it forever.** | A rotation design has to answer what happens to entries already published under the old key, which is exactly why `shareGroup` refuses to re-key today. |
| `SYNC-F20` | **A group with no admin is reachable by adoption and unrepairable.** `adoptGroup` inserts with `created_by` NULL and fills it only if the creator resolves locally; if the roster also carries no admin, nobody on that device can ever edit the budget, membership, roles, the name, or delete it. The launch invariants deliberately never guess a creator. | Either a repair path, or an egalitarian mode that makes "no admin" a **supported state** rather than an accident. |
...
| `D-11` | **Roster recovery self-heals only while the roster and the entry needing it fall inside one page (200).** In practice they always do, because `drainRosters` runs before the entry drain in the same sync. Recorded because it is the only shape that could stall. | `OPEN` |
...
| ~~**Multi-device sync (S2)**~~ / ~~**Shared groups (S3)**~~ | **Built.** Device identity, per-group keys, the outbox, the server, the transport and sharing a group are all complete end to end. What is *not* proven is in §5 and in `RELEASE_CHECKLIST.md` §3.1 — **sync has never run on a phone** | — |
