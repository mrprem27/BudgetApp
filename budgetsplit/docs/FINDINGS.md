...
**51 entries: 9 answered and kept, 35 still open, 7 `BLOCKED` outside the codebase** (`DQ-80`–`DQ-86`,
...
| `DQ-94` | **Merge two ledgers at first sign-in?** A phone with data signing into an account that already has data. The ids never collide (uuids), but each side has its own `is_me`, personal group and category list, and the same real expense may exist twice. | No merge in v1: **"Use my account"** (this phone exported to a file first, then replaced) or **"Not now"** (sign out). | The first person who uses the app on two phones before signing in. |
