...
> found. Live successor: `SYSTEM.md` — §6 features, §10 complexity, §11 decisions.
>
...
| F-16 | **Recurring rules** | Repeating transactions that materialize into real editable rows, with pause/resume/end/skip. | working | `src/db/queries/recurring.ts`, `src/lib/recurrence.ts`, `app/recurring/[id].tsx`, `app/plan/recurring.tsx` | `recurring` |
...
| ~~**ISS-04**~~ | ~~medium~~ | ✅ fixed | ~~**The onboarding persona question has no effect.** The choice is stored to `onboarding_intent` and never read. The code acknowledges it.~~ **Closed by Pass 4** (§4.6 above) — `lib/personaDefaults.ts` now maps intent → a sparse flag patch, applied inside `finalizeOnboarding`. | was `Onboarding.tsx:169-171`; no reader outside `settings.test.ts` |
