...
| ~~**V2-09**~~ ✅ | ~~`colors.textMuted` 2.98:1 on `bgCard`, below WCAG AA, on the app-wide caption token (`N1`)~~ | Fixed: `#7C918E`, 5.02:1 on bgCard. `contrast.test.ts` guards it | **M** |
| ~~**V2-13**~~ ✅ | ~~Gemini free tier is app-wide not per-user, cut 50-80% in late 2025; the documented Mistral fallback is not implemented (§19)~~ | Fixed: automatic on-device fallback. Mistral deliberately **not** built — a second vendor to solve what `device` solves offline | **M** |
...
| ~~**V2-15**~~ ✅ | ~~No notification deep-link routing; verified no `addNotificationResponseReceivedListener` anywhere. §18 already says "worth adding"~~ | Fixed: routes derived from the reminder id in `notificationRoutes.ts` | **L** |
...
| ~~**V2-20**~~ ✅ | ~~±24 h duplicate detection runs on Quick Add but not on the Review commit path~~ | Fixed: `findDuplicatesAmong`, batch names a count rather than prompting per row | **L** |
...
| ~~**V2-12**~~ ✅ | ~~Total Money conflates spendable cash, investments and credit; the Available-Money vs Net-Worth split is agreed (`COMPETITIVE_ANALYSIS.md` §7) and needs no server~~ | Split into Available Money (hero) + Net Worth — cheaper before users build muscle memory | **M** |
