...
| Item | Reason |
|---|---|
| Splitwise's Pay-by-Bank/Tink integration or Splitwise Pay (**app-custodied** money movement) | Requires becoming a financial intermediary with licensing/custodial risk — the clearest violation of the local-first/no-backend identity. **Narrowed 2026-08-05:** this entry is about rails where the *app* is a party to the transfer. A **UPI intent handoff** is a different category and shipped as `V2-03` — the app builds a `upi://pay` URI, the OS opens the user's own UPI app, and money moves peer-to-peer between the two users' bank accounts. No licence, no server, no float, nothing leaving the device but a locally-consumed URI. |
