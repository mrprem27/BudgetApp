...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    items: [
      { icon: 'mic', color: colors.accent, title: 'Three commands, and nothing to configure', body: '\u201cHey Siri, expense\u201d for spending, \u201cincome\u201d for money in, \u201ctransfer\u201d for paying someone back. Siri asks what it was, you answer, and BudgetSplit opens with the amount, category and date already filled in \u2014 you check it and tap Save. Which command you say is how it knows the kind, so that part is never guessed from your words. Install them once from Settings \u2192 Voice entry; there is nothing to set up afterwards. There is also the microphone on your keyboard on the Add screen, which needs no setup at all.' },
      { icon: 'edit-3', color: colors.accent, title: 'Where your words end up', body: 'Say only an amount and a category (\u201cfour fifty groceries\u201d) and nothing else is stored. Say more (\u201c450 zomato biryani\u201d) and the extra words become the title, which is what works out the category \u2014 exactly as if you had typed them. A long phrase titles itself with the first few words and puts the rest in the note, so nothing you said is thrown away.' },
      ...
      { icon: 'zap', color: colors.income, title: 'Nothing saves without you seeing it', body: 'The shortcut hands the phrase to BudgetSplit, which opens on the Add screen with everything filled in \u2014 so a mishearing is visible before it becomes a transaction, and your balance never moves behind your back. If the phrase sounded shared, the group picker is already open, because that is the one thing nobody can guess for you.' },
      { icon: 'check-circle', color: colors.income, title: 'What gets saved on its own', body: 'Any phrase with an amount that was clearly yours alone. An unrecognised description lands in Other rather than being refused \u2014 the same thing that happens when you type it \u2014 so voice is never stricter than the keyboard. The words you said stay on the row, so you can always see what was heard.' },
      { icon: 'inbox', color: colors.settle, title: 'What waits for you in Review', body: 'Two things: a phrase with no amount in it, and anything that sounded shared \u2014 either because you said split, group, with or owe, or because you named one of your actual groups. Those land in your Review inbox under \u201cSaid out loud\u201d with a line saying why. Nothing is lost and nothing is guessed.' },
      { icon: 'users', color: colors.coral, title: 'Splitting and settling by voice', body: 'Say it the normal way \u2014 \u201ctwelve hundred dinner with Rohan\u201d \u2014 and it is captured without the app opening, then waits in Review for you to pick who shares it. Saying split, group, with or owe, or naming one of your groups, is what holds it back; who shares a cost is a decision only you can make, so it is never guessed. \u201cSettle up\u201d works the same way: the amount and the person are already filled in and you confirm the direction.' },
      ...
      { icon: 'alert-circle', color: colors.healthAmber, title: 'What it will get wrong', body: 'Indian English handles words like chai and rupaye well, but a full Hinglish sentence will misfire sometimes \u2014 that is why the amount and category are always shown before anything is trusted. Expenses and income file themselves; settlements always wait in Review, because which way the money went is one mis-hearing away from being backwards. Itemized bills still need the normal screens.' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: 'target', color: colors.settle, title: 'Goals', body: 'Create goals with a target amount, priority and icon. Fund a goal directly from your Cash available (swipe or enter an amount). Each shows a progress bar, what’s left, and — if you set an auto-save amount — an estimated “done by” date. Withdrawing returns money to your cash.' },
      { icon: 'refresh-cw', color: colors.healthAmber, title: 'Auto-save', body: 'Give a goal a fixed amount and cadence (e.g. ₹5,000 monthly) and the app funds it from your Cash available on schedule. Locked and higher-priority goals are protected if cash runs short.' },
      { icon: 'alert-triangle', color: colors.expense, title: 'Overspending', body: 'If Cash available goes negative, the app automatically pulls from your lowest-priority goals first (your drag order) to cover it, and shows a notice with Undo. Locked goals and investments are never touched.' },
